...
private:
  ...
  //Store video and data results
  VideoWriter record_vid;
  VideoWriter record_vid_overlay;
  ofstream two_d_green;
  ofstream two_d_blue;
  ofstream three_d_green;
  ofstream three_d_blue;
...
public:
  ...
  handsTracker ()
  {
    ...
    only_hands.create(img_ht, img_wd, CV_8UC3);
    

    //open files
    two_d_green.open("src/data/greenH_2d.csv");
    two_d_blue.open("src/data/blueH_2d.csv");
    three_d_green.open("src/data/greenH_3d.csv");
    three_d_blue.open("src/data/blueH_3d.csv");
    
    //recorder
    record_vid.open("src/data/hands_rec.avi", CV_FOURCC('P','I','M','1'),30.0, 
		    img_of_interest.size());
    record_vid_overlay.open("src/data/hands_rec_blobs.avi", CV_FOURCC('P','I','M','1'),30.0, img_of_interest.size());
    
    //check if opened
    if (!record_vid.isOpened() || !record_vid_overlay.isOpened()){cout<<endl<<" Unable to open Video"<<endl; exit(-1);}
    ...
    //3d points
    purple_hand = nh.advertise<geometry_msgs::PoseStamped> ("right_hand", frames_to_hold);
    green_hand = nh.advertise<geometry_msgs::PoseStamped> ("left_hand", frames_to_hold);
...
void find_blobs(const Mat &binary, vector < vector<Point2i> > &blobs, int* max_idx_out, int* max_area_out)
{
  ...
  bool get_centroid(Point2f*  out_blob_centroid, double area_thresh, bool isGreen, Point3f* centroid_3d)
  {
	...
	accumulator_set<double, features<tag::median> > x_acc_2d, y_acc_2d;
    	...
    	if(cur_pc)
	  {
		...
		//debug
		//cout<< "Point = "<<pt_2d<<endl; 

		PRGB pt = cur_pc->at(pt_2d.x, pt_2d.y);
		
		if(pt.x == pt.x && pt.y == pt.y && pt.z == pt.z) {
		  x_acc(pt.x); y_acc(pt.y); z_acc(pt.z); 
		  x_acc_2d(pt_2d.x); y_acc_2d(pt_2d.y);
		}
		
		//point belongs to largest blob
		only_hands.at<Vec3b>(pt_2d.y, pt_2d.x) = img_of_interest.at<Vec3b>(pt_2d.y, pt_2d.x);
	      }
        
	    Point3f temp_3d; Point2f temp_2d;
	    temp_3d.x=median(x_acc); temp_3d.y=median(y_acc); temp_3d.z=median(z_acc);
	    temp_2d.x=median(x_acc_2d); temp_2d.y=median(y_acc_2d);
  ...
  void publish_centroid(Point3f centroid, bool found, ros::Publisher pub)
  {
    ...
    geometry_msgs::PoseStamped msg2pub;
    ...
    msg2pub.header.frame_id = frame_of_reference;
    msg2pub.pose.position = msg;
    msg2pub.pose.orientation = init_quaternion;
    pub.publish(msg2pub);
  ...
  void frame_process()
  {
	...
	if (!found_blue){
	  //debug
	  cout<<endl<<"This just happened!"<<endl;
	  two_d_blue << "inf\n";three_d_blue << "inf\n";
	  cout<<endl<<"No real problemo"<<endl;}
	else
	  {
	    two_d_blue << blue_centroid.x << ';' << blue_centroid.y << '\n';
	    three_d_blue << blue_centroid_3.x << ';' << blue_centroid_3.y 
			 << ';' << blue_centroid_3.z << ';'<< '\n';
	  }
	
	if (!found_green){two_d_green << "inf\n";three_d_green << "inf\n";}
	else
	  {
	    two_d_green << green_centroid.x << ';' << green_centroid.y << '\n';
	    three_d_green << green_centroid_3.x << ';' << green_centroid_3.y 
			  << ';' << green_centroid_3.z << ';'<< '\n';
	  }
  ...
  void kin_pc_sub(sensor_msgs::PointCloud2::ConstPtr pc_msg) 
  {
    ...
    int tot_rows = cur_pc->height, tot_cols = cur_pc->width;
    
    //debug
    if(tot_rows!=480 || tot_cols!=640){cout<<"Incorrect Size"; exit(-1);}
	...
	//cout<<"reach:"<<r<<endl;
      ...
      //debug
      //    cout<< "Point cloud got!"<<endl;
      
      //record
      record_vid << img_of_interest;
    
      //debug
      //cout << "Frame added to vid"<<endl;

      //clear only hands image
      only_hands = Scalar(255,255,255);
      ...
      //debug
      //cout<< "Frame process begins"<<endl; 
	...
	ros::shutdown(); exit(-1);
      ...
      //debug
      //cout<< "Frame process ends"<<endl; 

      //record only hands
      record_vid_overlay << only_hands;
    
      //    char c = waitKey(33);
      //cout<< 'ESC'<<endl;
      //if (c=='m'){cout<<"ATLEAST MET"<<endl; ros::shutdown(); exit(-1);}
    
      //debug
      //cout<< "only hands frame recorded"<<endl; 
