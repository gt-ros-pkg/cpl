...
void find_blobs(const Mat &binary, vector < vector<Point2i> > &blobs, int* max_idx_out, int* max_area_out)
{
  ...
  void kin_pc_sub(sensor_msgs::PointCloud2::ConstPtr pc_msg) 
  {
    ...
    ++handsTracker::frame_no;

    //debug
    //    if(tot_rows!=480 || tot_cols!=640){cout<<"Incorrect Size"; exit(-1);}

    // if (cur_pc){
      string file_name = boost::lexical_cast<string>(handsTracker::frame_no);
      file_name += ".pcd";
      string path = "src/data/pcl/cur_run/";

      double frequency = getTickFrequency();
      double start = getTickCount();
      pcd_writer.write (path+file_name, pc_msg);
      double time_el = ((double)getTickCount()-start)/frequency;
      
      //debug
      cout<<"Time to store = "<<time_el<<endl;
