...
bool get_centroid(const Mat img_of_interest, Point2f*  out_blob_centroid, double area_thresh, tf::Transform& median_tf, bool isGreen, Point3f* centroid_3d)
{
...
//it publishes (-1,-1,-1) in case hand was not found
...
void frame_process(Mat img_of_interest)
{
  ...
  ros::Publisher purple_hand = nh.advertise<geometry_msgs::Point>
    ("right_hand", 0);
  ros::Publisher green_hand = nh.advertise<geometry_msgs::Point>
    ("left_hand", 0);*/
  ...
  char c;
	...
	if(!img_of_interest.empty())
	{
	  ...
	  bool found_blue = get_centroid(img_of_interest, &blue_centroid, blue_area_thresh, blue_median_tf, false, &blue_centroid_3);
	  bool found_green = get_centroid(img_of_interest, &green_centroid, green_area_thresh, green_median_tf, true, &green_centroid_3);
