  ...
  if ((max_idx>-1) && (max_area>area_thresh))
    {
      ...
      *out_blob_centroid = cont_centr;
      ...
      if(cur_pc) {
        cout << endl << "h" << endl;
        cout << endl << cur_pc-> << endl;
      }
      if(cur_pc && (fabs((cur_pc->header.stamp - last_img_time).toSec()) < 0.2)) {
...
int main(int argc, char **argv)
{
  ...
  while(ros::ok())
      ...
      else
	{
	  ...
	  double blue_area_thresh = 20.0, green_area_thresh = 20.0;
    ...
    tf::Transform blue_median_tf, green_median_tf;
	  bool found_blue = get_centroid(img_of_interest, blue_method, blue_means, 
					 blue_var, blue_cov_mat_inv, &blue_centroid, blue_area_thresh, blue_median_tf);
	  bool found_green = get_centroid(img_of_interest, green_method, green_means, 
					  green_var, green_cov_mat_inv, &green_centroid, green_area_thresh, green_median_tf);
