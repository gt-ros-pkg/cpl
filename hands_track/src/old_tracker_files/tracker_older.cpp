...
private:
  ...
  Mat temp_h, temp_s, temp_v, label_image;
  //Mat temp_green, temp_blue;
  // Mat thresh_green, thresh_blue;
...
public:
  ...
  handsTracker ()
  {
    ...
    label_img(img_ht, img_wd, CV_32FC1);
    //new_bin.create(img_ht, img_wd, CV_8UC1);
    temp_h.create(img_ht, img_wd, CV_8UC1);
    temp_s.create(img_ht, img_wd, CV_8UC1);
    temp_v.create(img_ht, img_wd, CV_8UC1);
    /*    diff_h.create(img_ht, img_wd,  CV_32FC1);
    temp_green =  Mat::ones(img_ht, img_wd, CV_32FC1) * green_mean * 180.0; 
    temp_blue =  Mat::ones(img_ht, img_wd, CV_32FC1) * blue_mean * 180.0;
    thresh_green =  2*hard_th_green*Mat::ones(img_ht, img_wd, CV_32FC1);  
    thresh_blue =  2*hard_th_blue*Mat::ones(img_ht, img_wd, CV_32FC1);*/
...
void find_blobs(const Mat &binary, vector < vector<Point2i> > &blobs, int* max_idx_out, int* max_area_out)
{
  ...
  bool get_centroid(Point2f*  out_blob_centroid, double area_thresh, bool isGreen, Point3f* centroid_3d)
  {
    ...
    for( size_t idx = 0; idx < num_contours; idx++ )
      {
	img_of_interest.at<Scalar>(blobs[max_idx][idx].y, blobs[max_idx][idx].x) = color;
      }
