...
from ur_cart_move.ur_cart_move import load_ur_robot, RAVEKinematics
...
def main():
    ...
    kin = RAVEKinematics(load_ik=False)
    #arm1, kin, arm_behav = load_ur_robot(topic_prefix="/sim1", timeout=0.)
                    ...
                    for i5 in range(4):
                        for i6 in range(4):
                            ...
                            if rospy.is_shutdown():
                                sys.exit()
                            if diff > 0.05:
                                print np.array(inverse_kin(x, UR10_A, UR10_D, UR10_L, q[5]))
